...
export default function ProjectCard({ project, inProgress = false }: Props) {
    ...
    return (
        ...
        >
            ...
            <div className="p-4">
                {inProgress && (
                    <span className="text-xs text-blue-400 font-semibold uppercase">{t.inProgressLabel}</span>
                )}
                <h3 className="text-lg font-semibold text-white mb-1">{project.title}</h3>
                <p className="text-sm text-neutral-400 mb-2">{project.description[lang]}</p>
